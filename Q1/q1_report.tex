...
It should be noted that whitespace-based splitting behaves 
differently across the three languages. Unlike English and Urdu, 
Chinese does not use spaces between words, so the splitting step 
treats entire Chinese sentences as single units. Since Chinese 
characters are themselves meaningful units, the subsequent 
character-level decomposition remains a reasonable approximation 
for this corpus.
...
\(K\). For example, at K=300, BPE had already learned complete tokens such 
as \textit{tokenization} and \textit{language}, while WordPiece had 
not yet merged any English character pairs at all. This is because BPE's raw frequency criterion merges them early.
